...
<div class="flex min-h-screen items-center justify-center bg-slate-950 px-6">
  <div class="w-full max-w-sm">
    <div class="mb-8 text-center">
      <div class="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-lg bg-indigo-600">
    ...
    <form on:submit|preventDefault={submit} class="space-y-5 rounded-xl border border-slate-800 bg-slate-900/60 p-6">
      <input
        bind:value={email}
        type="email"
        required
      ...
      <input
        ...
        class="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-white"
      />
      <button disabled={processing} class="w-full rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-medium text-white disabled:opacity-50">
